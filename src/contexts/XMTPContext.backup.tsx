...
export const XMTPProvider: React.FC<XMTPProviderProps> = ({ children }) => {
  ...
  const backgroundSyncManager = useCallback(async () => {
    ...
    try {
      ...
      // WORKING PATTERN: Simple, effective conversation loading
      console.log('[XMTP] 🔍 SIMPLE CONVERSATION SYNC - Loading all conversations');
      ...
      try {
        ...
        console.warn('[XMTP] Sync warning (non-critical):', syncError);
      }
      
      // Step 2: Get ALL conversations (working pattern)
      const convos = await client.conversations.list();
      console.log(`[XMTP] 📋 Raw conversations loaded: ${convos.length} found`);
      
      // Step 3: Log each conversation for debugging
      for (const conv of convos) {
        const convInfo = {
          id: conv.id,
          peerAddress: 'peerAddress' in conv ? conv.peerAddress : 'N/A',
          isDeleted: deletedConversationIds.has(conv.id),
          senderAddress: address
        };
        console.log(`[XMTP] 📋 Background sync found conversation:`, convInfo);
      }
      
      // Step 4: Filter and set conversations
  ...
  useEffect(() => {
    ...
    // TESTING: Very aggressive sync intervals to catch inbound conversations
    const getSyncInterval = () => {
      switch (connectionQuality) {
        case 'excellent': return 3000;  // Every 3 seconds for testing
        case 'good': return 5000;       // Every 5 seconds
        case 'poor': return 8000;       // Every 8 seconds  
        case 'offline': return 15000;   // Every 15 seconds
        default: return 3000;           // Default to very frequent sync for testing
      }
    };
  ...
  // WORKING REFERENCE PATTERN: Simple conversation discovery that actually works
  const loadConversationsWithRetry = useCallback(async (maxRetries: number = 2, delay: number = 1000): Promise<XMTPConversation[]> => {
    if (!client) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[XMTP] 🔄 Simple conversation discovery attempt ${attempt}/${maxRetries}`);
        
        // WORKING PATTERN: Force a comprehensive sync first
        try {
          await client.conversations.sync();
          console.log(`[XMTP] ✅ Basic sync completed on attempt ${attempt}`);
        } catch (syncError) {
          console.warn(`[XMTP] Sync warning on attempt ${attempt}:`, syncError);
        }

        // Small delay for sync propagation
        await new Promise(resolve => setTimeout(resolve, 500));

        // WORKING PATTERN: Just get ALL conversations without filtering by consent state initially
        const allConversations = await client.conversations.list();
        console.log(`[XMTP] 📋 Raw conversation discovery: ${allConversations.length} total found`);
        
        // Debug: Log details about each conversation
        for (const conv of allConversations) {
          const convInfo = {
            id: conv.id,
            peerAddress: 'peerAddress' in conv ? conv.peerAddress : 'N/A',
            createdAt: 'createdAt' in conv ? conv.createdAt : 'N/A',
            state: 'state' in conv ? (conv as any).state : 'unknown'
          };
          console.log(`[XMTP] 📋 Found conversation:`, convInfo);
        }
        
        return allConversations as XMTPConversation[];
      } catch (error) {
        console.error(`[XMTP] ❌ Simple discovery failed on attempt ${attempt}:`, error);
  ...
  const loadConversations = useCallback(async () => {
    if (!client || !deletedIdsLoaded) {
      console.log('[XMTP] Skipping loadConversations - waiting for client/deleted IDs');
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      // Simple filtering - just remove deleted conversations
      const filteredConvos = convos.filter(c => !deletedConversationIds.has(c.id));
      
      console.log('[XMTP] 🔧 SIMPLE: Setting conversations from loadConversations:', { 
        totalFound: convos.length, 
        afterFilter: filteredConvos.length,
        walletAddress: address
      });
      
      // Show details about each conversation for debugging
      for (const conv of filteredConvos) {
        const convInfo = {
          id: conv.id,
          peerAddress: 'peerAddress' in conv ? conv.peerAddress : 'N/A',
          isFromMe: 'peerAddress' in conv ? conv.peerAddress === address : false
        };
        console.log(`[XMTP] 📋 Setting conversation in state:`, convInfo);
      }
      
      safeSetConversations(filteredConvos);
  ...
  }, [client, deletedIdsLoaded, loadConversationsWithRetry, deletedConversationIds, address, safeSetConversations]);
