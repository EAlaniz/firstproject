...
export const XMTPProvider: React.FC<XMTPProviderProps> = ({ children }) => {
  ...
  const handleXMTPRegistration = async (signer: Signer) => {
    // Skip signature test in mini app environment
    const isMiniApp = !!(window as any)?.farcaster?.version ||
    ...
    if (!isMiniApp) {
      // Debug: log signature for troubleshooting (only in web mode)
      try {
        ...
        if (e.code === 4001 || e.message.includes('user rejected') || e.message.includes('ACTION_REJECTED')) {
          throw new Error('XMTP initialization was cancelled. Please try again and approve the signature request to enable messaging.');
        }
        
        throw new Error('Wallet signature test failed. Please ensure you are using MetaMask or Coinbase Wallet.');
      }
    } else {
      console.log('Mini app mode: skipping signature test');
    }
    ...
    try {
      ...
      // Handle user rejection specifically
      if (error.code === 4001 || error.message.includes('user rejected') || error.message.includes('ACTION_REJECTED')) {
        throw new Error('XMTP initialization was cancelled. Please try again and approve the signature request to enable messaging.');
      }
